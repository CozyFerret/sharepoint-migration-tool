...
# Import the fixers that are definitely available
...
# Try to import PermissionFixer, but don't fail if it doesn't exist
try:
...
class DataProcessor(QObject):
    ...
    def __init__(self, config=None):
        ...
        self.scanner = None  # Initialize on-demand to avoid scanner issues
        ...
        # Fixed initialization of PIIDetector without arguments
    ...
    def start_scan(self, root_path, scan_options=None, callbacks=None):
        ...
        try:
            ...
            if 'progress' in callbacks:
                # Ensure any previous connections are disconnected
                try:
                    self.scanner.progress_updated.disconnect()
                except:
            ...
            if 'error' in callbacks:
                callbacks['error'](f"Error starting scan: {str(e)}")
    ...
    def _scan_completed(self, results, callback=None):
        ...
        try:
            ...
            if callback:
                # Send empty DataFrame to avoid crashes
    ...
    def _process_scan_results(self, results):
        ...
        try:
            ...
            # Create DataFrame or default to empty DataFrame
            if file_list:
                self.scan_data = pd.DataFrame(file_list)
            else:
                self.scan_data = pd.DataFrame()
        except Exception as e:
            logger.error(f"Error in _process_scan_results: {e}")
    ...
    def analyze_data(self, feature_flags=None, callbacks=None):
        ...
        try:
            if self.scan_data is None or len(self.scan_data) == 0:
                ...
                if callbacks and callbacks.get('error'):
                    callbacks['error']("No data to analyze")
                return
            ...
            if callbacks and callbacks.get('error'):
                callbacks['error'](f"Error starting analysis: {str(e)}")
    ...
    def _analyze_name_issues(self, callback=None):
        ...
        try:
            ...
            # Create empty DataFrame for the result to avoid None errors
            self.analysis_results['name_issues'] = pd.DataFrame()
    ...
    def _analyze_path_issues(self, callback=None):
        ...
        try:
            ...
            # Create empty DataFrame for the result to avoid None errors
            self.analysis_results['path_issues'] = pd.DataFrame()
    ...
    def _analyze_duplicates(self, callback=None):
        ...
        try:
            ...
            # Create empty DataFrame for the result to avoid None errors
            self.analysis_results['duplicates'] = pd.DataFrame()
    ...
    def _analyze_pii(self, callback=None):
        ...
        try:
            ...
            # Create empty DataFrame for the result to avoid None errors
            self.analysis_results['pii'] = pd.DataFrame()
    ...
    def start_cleaning(self, target_folder, clean_options, callbacks=None):
        ...
        try:
            if not self.analysis_results:
                ...
                if callbacks and callbacks.get('error'):
                    callbacks['error']("No analysis results to process")
                return
            ...
            if callbacks and callbacks.get('error'):
                callbacks['error'](f"Error starting cleaning: {str(e)}")
    ...
    def _cleaning_completed(self, cleaned_files, callback=None):
        ...
        try:
            ...
            logger.error(f"Error in _cleaning_completed: {e}")
            if callback:
                callback({})  # Send empty dict to avoid crashes
    ...
    def clean_and_upload(self, clean_options, sharepoint_config, callbacks=None):
        ...
        try:
            ...
            logger.error(f"Error in clean_and_upload: {e}")
            if callbacks and callbacks.get('error'):
                callbacks['error'](f"Error starting clean and upload: {str(e)}")
    ...
    def _upload_to_sharepoint(self, cleaned_files, sharepoint_config, callbacks):
        ...
        try:
            ...
            logger.error(f"Error in _upload_to_sharepoint: {e}")
            if callbacks.get('error'):
                callbacks['error'](f"Error during SharePoint upload: {str(e)}")
    ...
    def fix_issues(self, params):
        ...
        try:
            ...
            # Try to create permission fixer, but don't fail if it's not available
            permission_fixer = None
            if strategies.get('permissions'):
                try:
                    permission_fixer = PermissionFixer()
                except Exception as e:
                    logger.warning(f"PermissionFixer not available: {e}")
            ...
            for file_path in all_files:
                ...
                try:
                    ...
                    if deduplicator and hasattr(deduplicator, 'strategy') and deduplicator.strategy == "Rename All Duplicates":
                        for group_idx, group in enumerate(duplicate_groups):
                            if file_path in group:
                                ...
                                try:
                                    ...
                                    logger.error(f"Error fixing duplicate file: {e}")
                                break
                    else:
